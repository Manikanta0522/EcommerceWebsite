...
public class UserDAO {
    ...
    public void save(User u) throws SQLException {
        String sql = "INSERT INTO users(fullname,email,password_hash,phone) VALUES(?,?,?,?)"; // updated column
        try (Connection c = DBUtil.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            p.setString(1, u.getFullname()); // updated getter
        ...
        try (Connection c = DBUtil.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            ...
            try (ResultSet r = p.executeQuery()) {
                if (r.next()) {
                    User u = new User();
                    u.setId(r.getInt("id"));
                    u.setFullname(r.getString("fullname")); // updated column
